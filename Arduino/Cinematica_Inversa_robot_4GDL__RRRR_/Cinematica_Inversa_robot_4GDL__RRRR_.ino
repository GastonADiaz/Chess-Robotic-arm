...
CoordenadasGlobales coordenada_de_muerte[1]= {                // Creo una instancia de 'CoordenadasGlobales' llamada 'coordenada_de_muerte' con un tamaño de 1 y la inicializo.
    {-170, 90, 40, 90}                                        // Inicializo la coordenada de muerte con los valores de las coordenadas de la cajita de muerte.
};
...
CoordenadasGlobales coordenadas_del_tablero[64]= {            // Creo una instancia de 'CoordenadasGlobales' llamada 'coordenadas_del_tablero' con un tamaño de 64 casillas y la inicializo.
    {92, 240, 20, 90}, {65, 240, 20, 90}, {34, 240, 25, 90}, {12, 240, 25, 90}, {-12, 240, 25, 90}, {-42, 240, 25, 90}, {-65, 240, 25, 90}, {-95, 240, 25, 90},     // A1 - H1 
    {92, 210, 18, 90}, {65, 210, 15, 90}, {34, 210, 15, 90}, {8, 210, 15, 90}, {-12, 210, 16, 90}, {-42, 208, 15, 90}, {-65, 212, 20, 90}, {-95, 214, 20, 90},      // A2 - H2
    {83, 178, 12, 90}, {60, 178, 12, 90}, {34, 178, 16, 90}, {9, 178, 15, 90}, {-12, 178, 16, 90}, {-40, 178, 15, 90}, {-65, 178, 16, 90}, {-93, 178, 15, 90},      // A3 - H3
    {82, 155, 12, 90}, {60, 155, 14, 90}, {32, 155, 14, 90}, {7, 155, 14, 90}, {-15, 152, 14, 90}, {-42, 152, 14, 90}, {-65, 152, 14, 90}, {-90, 148, 15, 90},      // A4 - H4
    {80, 128, 13, 90}, {58, 130, 12, 90}, {28, 126, 10, 90}, {7, 126, 10, 90}, {-15, 128, 15, 90}, {-35, 126, 15, 90}, {-60, 126, 13, 90}, {-85, 126, 15, 90},      // A5 - H5
    {73, 104, 10, 90}, {54, 104, 10, 90}, {29, 104, 8, 90}, {8, 100, 6, 90}, {-13, 100, 6, 90}, {-35, 100, 6, 90}, {-60, 100, 10, 90}, {-83, 100, 11, 90},          // A6 - H6
    {73, 85, 6, 90},   {45, 72, 0, 90},   {23, 70, -3, 90},   {8, 65, 0, 90},   {-10, 70, 0, 90},    {-29, 70, 0, 90},   {-54, 76, 6, 90},   {-79, 76, 7, 90},       // A7 - H7
    {74, 60, 25, 90},  {52, 56, 15, 90},  {32, 55, 15, 90},  {12, 52, 12, 90},  {-8, 60, 15, 90},  {-26, 52, 10, 90},  {-48, 60, 14, 90},  {-78, 60, 26, 90}        // A8 - H8  // Solo funciona bien con q2 de caida en 0
};
...
CoordenadasGlobales coordenadas_de_la_cajita[34] = {          // Creo una instancia de 'CoordenadasGlobales' llamada 'coordenadas_de_la_cajita' con un tamaño de 34 celdas y la inicializo.
    {130, 15, 20, 90}, {132, 37, 15, 90}, {132, 55, 20, 90},{133, 77, 20, 90}, {139, 98, 18, 90}, {137, 115, 18, 90},{137, 134, 20, 90}, {138, 164, 22, 90},                                // Fila 1
...
int Gripper_open = 27 ;                             // Declaro una variable de tipo int para el ángulo de apertura del gripper. Pongo 27 para una apertura moderada.     
int Gripper_closed = 4 ;                            // Declaro una variable de tipo int para el ángulo de cierre del gripper. Pongo 4 para asegurar un cierre firme. Para mayor seguridad usar 5 (100% seguro que no falla)
int Tiempo_para_velocidad_servo_lenta = 3 ;         // Declaro una variable de tipo int para el tiempo de movimiento lento del servo. Pongo 3 para movimientos lentos.
int Tiempo_para_velocidad_servo_rapida = 40 ;       // Declaro una variable de tipo int para el tiempo de movimiento rápido del servo. Poner 40 para movimientos rápidos.(con 10 funciona perfecto, aunque no tan rapido)
int Tiempo_para_velocidad_home = 20 ;               // Declaro una variable de tipo int para el tiempo de movimiento del servo a la posición home. Pongo 20 para movimientos moderados.
...
int Angulo_ajuste_q3_caida = 11;                    // Declaro una variable de tipo int para ajustar y compensar el ángulo Q2 en caso de caída. 11 para casillas finales
...
void setup() {
  ...
  Asignacion_de_pines_a_servos(8, 9, 10, 11, 12, 6);  // Pines donde conecto los servos. (serv1,serv2_1,serv3,serv4,serv5,serv2_2)    // El ultimo parametro es para el servo adicional en la articulacion 2, es opcional
 ...
 // Mover_pieza("H8","H6");
  ...
  //Mover_pieza("A8","A7");
  //Comer_pieza("B6","B8");
  //Ejecutar_movimiento(0, 230, 30, 90);              // Recordar descomentar para usar esta funcion. (Que basicamente es la cinematica inversa)
}
...
void loop() {
  ...
  coordenadas_del_tablero_recibidas_desde_py(); // Funcion que se usa con el vector de coordenadas por ejemplo:['A1','A2','5']
  //Punto_a_punto_py();
}
...
void Movimiento_directo(double Q1_grados, double Q2_grados, double Q3_grados, double Q4_grados) {                
...
void Comer_pieza(const char* posicion_inicial, const char* posicion_final) {            // Lo mismo que la funcion de 'mover pieza' pero agregando una secuencia mas, para cuando come una pieza, es decir llevarla a la coordenada de muerte a la pieza comida.
    ...
    Mover_brazo_a_la_velocidad_lenta(Q1_grados, Q2_grados, Q3_grados, Q4_grados);
    Cerrar_el_gripper(Gripper_closed);
    delay(tiempo);
...
void Mover_brazo_a_la_velocidad_lenta(double Q1_grados, double Q2_grados, double Q3_grados, double Q4_grados) {
  ...
  servo4.write(qh4 - abs(Q4_grados) -Angulo_ajuste_Gripper,Tiempo_para_velocidad_servo_lenta+3,true);                           // Mando al servo 4 que se mueva , con velocidad lenta , pero con +3 la velocidad gripper...para movimiento lineal y esperando a que termine antes de iniciar el siguiente movimiento.
...
void Abrir_el_gripper(int Gripper_open){                                              // Defino una función para abrir el gripper utilizando el valor proporcionado.
  delay(2000);                                                                        // Hago una pausa de 2 segundos antes de abrir el gripper.
...
void Cerrar_el_gripper(int Gripper_closed){                                           // Defino una función para cerrar el gripper utilizando el valor proporcionado.
  delay(2000);                                                                        // Hago una pausa de 2 segundos antes de cerrar el gripper.
